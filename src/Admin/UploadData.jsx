    ...
    <div className="w-full flex flex-col pt-20 items-center px-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 w-full max-w-5xl mb-10">
        {imageUploads.map((item) => (
          <Card key={item._id} item={item} setExpanded={setExpanded} handleDelete={handleDelete} />
        ))}
      </div>
        ...
        >
          ...
          >
            {expanded.videoPath ? (
              ...
              <img src={expanded.url} alt={expanded.title} className="w-full rounded-lg" />
            ) : null}
    ...
    <div className="relative border rounded-lg shadow overflow-hidden">
      {fileUrl ? (
        item.videoPath ? (
          ...
          <img src={fileUrl} alt={item.title} className="w-full h-32 object-cover" />
        )
      ) : (
